...
\end{equation}
% The apparent degeneracy when $N = 0$ and $N = 2$ in figure \ref{fig012} is due to the fact that $\xi$ can be written in terms of $u$ as
% \begin{equation}
% 	\xi(s) = \frac{M_{A}^{2} + M_{B}^{2} - u}{2 M_{A} M_{B}} = - \xi(u),
% \end{equation}
% and since $\rho_{0}$ and $\rho_{2}$ are even functions of $\xi$, the trajectory functions must pass through both $s_{nl}$ and $u_{nl}$ values. On the other hand, $\rho_{1}$ is an odd function of $\xi$ and indeed under the replacement $\xi \rightarrow -\xi$ the trajectory function is reflected along the vertical axis and passes through the $u_{nl}$ values.
...
% However, this logarithm is responsible for breaking the degeneracy in figure \ref{figR0R}, which is not expected in the Regge limit, but is a property in the forward-JWKB limit.
